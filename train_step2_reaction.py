...
from evodiff.model_s2 import ByteNetLMTime
from evodiff.utils import Tokenizer
from torch.utils.data import Subset
from sequence_models.samplers import SortishSampler, ApproxBatchSampler
from sequence_models.datasets import UniRefDataset
from sequence_models.constants import MSA_ALPHABET
from evodiff.collaters import OAMaskCollater, D3PMCollater_s2
...
def main():
    ...
    parser.add_argument('--mini_run', type=str, default='True') # Set to True if running on subset of data

    # parser.add_argument('--mask', type=str, default='random')  # Set to True if running on subset of data
    parser.add_argument('--mask', type=str, default='blosum')  # Set to True if running on subset of data
    ...
    parser.add_argument('--checkpoint_freq', type=float, default=2)  # in minutes
...
def train(gpu, args):
    ...
    try:
        ...
        data_top_dir =r'/workspace/zhangzh/pUGTdb/3D_molecule_train/'
        ptjob = False
    data_dir = r'/workspace/zhangzh/pUGTdb/molecule_finetuning/reaction_fine_tune/'
    if args.mini_run:
    ...
    if args.mask == 'oadm':
        ...
        collater = D3PMCollater_s2(tokenizer=tokenizer, num_timesteps=diffusion_timesteps, Q=Q_t, Q_bar=Q_prod)
    else:
    ...
    # args.state_dict = './output/test_best_model_checkpoint.tar'
    # args.state_dict = None
    # args.state_dict = './output/checkpoint248127_my_data_fine_tune.tar'
    args.state_dict = './output/checkpoint9563_reaction_fine_tune_1_75.tar'
    # args.state_dict = './output/test_best_model_checkpoint_38m_random.tar'
    ...
    for p in model.embedder.fuser.parameters():
        p.requires_grad_(True)
    for p in model.embedder.cond_map.parameters():
        p.requires_grad_(True)
    ...
    def step(model, batch, train):
        if args.mask == 'blosum' or args.mask == 'random':
            src, src_onehot, timestep, tgt, tgt_onehot, Q, Q_bar, q,reaction_feats = batch
            q = q.to(device)
            Q = Q.to(device)
            Q_bar = Q_bar.to(device)
            src_onehot = src_onehot.to(device)
            tgt_onehot = tgt_onehot.to(device)
            reaction_feats = reaction_feats.to(device)
        else:
        ...
        with torch.amp.autocast(device_type='cuda'):
            outputs = model(src, timestep,mo_feats=reaction_feats, input_mask=input_mask.unsqueeze(-1))
            if args.mask == 'blosum' or args.mask == 'random':
